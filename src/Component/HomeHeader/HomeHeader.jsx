...
function HomeHeader({ user, selected }) {
  ...
  const collapseNavbar = () => {
    document.getElementsByClassName('homeHeader_logo')[0].style.display = 'block';
    document.getElementsByClassName('homeHeader_searchBack')[0].style.display = 'none';
    document.getElementsByClassName('searchBox')[0].style.display = 'none';
    document.getElementsByClassName('homeHeader_search')[0].style.display = 'block';
    document.getElementsByClassName('dropdown-content3')[0].style.display = 'none';
    document.getElementsByClassName('searchBox')[0].value = '';
  }

  const expandNavbar = () => {
    document.getElementsByClassName('homeHeader_logo')[0].style.display = 'none';
    document.getElementsByClassName('homeHeader_searchBack')[0].style.display = 'block';
    document.getElementsByClassName('homeHeader_search')[0].style.display = 'none';
    document.getElementsByClassName('searchBox')[0].style.display = 'block';
  }

  useEffect(() => {
    db.collection('users').onSnapshot(snapshot => {
      setUsers(snapshot.docs.map((doc) => doc.data())) // this statement over here gets all the users from the collection irrespective of the search term
    })// câu lệnh lấy tất cả user từ collection không phân biệt điều kiện tìm kiếm

    if (users !== undefined) {
      const finalUsers = users.filter(user => {
        return user.displayName.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1;// trả về các người dùng có index khác -1 dưới dạng kí tự thường
      })
  ...
  return (
    <div className='homeHeader'>
      ...
      <div className='homeHeader_pages'>
        <div className='home'>
          {
            selected ? (
      ...
      <div className='homeHeader_otherIcons'>
        ...
        <div className='round' onClick={renderProfile}>
          ...
          <svg fill="currentColor" viewBox="0 0 16 16" width="1em" height="1em" class="dropdownIcon"><g fill-rule="evenodd" transform="translate(-448 -544)"><path fill-rule="nonzero" d="M452.707 549.293a1 1 0 0 0-1.414 1.414l4 4a1 1 0 0 0 1.414 0l4-4a1 1 0 0 0-1.414-1.414L456 552.586l-3.293-3.293z"></path></g></svg>
          </i>
